...
# Energy range (in log₁₀ eV) over which the spectral quantities are plotted:
# 50 eV to 400 keV.
const LOG_EMIN = log10(50.0)
const LOG_EMAX = log10(4.0e5)

# Plot the four mass attenuation coefficient types versus energy for element `z`.
function plotMACs(z::Integer)
    colors, name = [ "red", "green", "blue", "orange" ], [ "[μ/ρ]ₚₑ", "[μ/ρ]ci", "[μ/ρ]ₜₒₜ", "[μ/ρ]ₖ" ]
    lpe = layer(loge->log10(mac(PhotoElectricMAC, z, 10.0^loge)), LOG_EMIN, LOG_EMAX, Theme(default_color = colors[1]))
    lci = layer(loge->log10(mac(CoherentIncoherentMAC, z, 10.0^loge)), LOG_EMIN, LOG_EMAX, Theme(default_color = colors[2]))
    ltot = layer(loge->log10(mac(TotalMAC, z, 10.0^loge)), LOG_EMIN, LOG_EMAX, Theme(default_color = colors[3]))
    lK = layer(loge->log10(mac(KMAC, z, 10.0^loge)), LOG_EMIN, LOG_EMAX, Theme(default_color = colors[4]))
    plot(lpe, lci, ltot, lK,
        Guide.title("Z = $z"),
        Guide.manual_color_key("Type", name, colors),
        Guide.xlabel("log₁₀(E) (eV)"), Guide.ylabel("log₁₀([μ/ρ]) (g/cm²)"),
        Coord.cartesian(xmin = LOG_EMIN, xmax = LOG_EMAX, ymin = -2.0))
end

# Plot the two form factors f₁ and f₂ versus energy for element `z`.
function plotFormFactors(z::Integer)
    colors, name = [ "red", "blue" ], [ "f₁(E)", "f₂(E)" ]
    lf1 = layer(loge->formfactors(z, 10.0^loge)[1], LOG_EMIN, LOG_EMAX, Theme(default_color = colors[1]))
    lf2 = layer(loge->formfactors(z, 10.0^loge)[2], LOG_EMIN, LOG_EMAX, Theme(default_color = colors[2]))
    plot(lf1, lf2,
        Guide.title("Z = $z"),
        Guide.manual_color_key("Type", name, colors),
        Guide.xlabel("log₁₀(E) (eV)"), Guide.ylabel("log₁₀(f(E)) (e/atom)"),
        Coord.cartesian(xmin = LOG_EMIN, xmax = LOG_EMAX, ymin = 0.0))
end


# Plot, per element, the number of tabulated edges and the highest edge index.
...
# Plot the nominal atomic weight against atomic number.
plotAtomicWeight() =
...
# Plot the cross-section factor (in barns/atom) against atomic number.
...
# Plot the nominal density against atomic number.
plotDensity() =
    plot(x = eachelement(), y = density.(eachelement()), #
    Guide.title("Nominal Density"), #
    Guide.xlabel("Z"), Guide.ylabel("g/cm³"))

# Plot both relativistic correction estimates against atomic number.
...
# Plot the nuclear Thomson correction against atomic number.
...
# Plot the jump ratios for the given shell across all elements.
